...
var logFile = 'styles.log';

function process(config) {
  var sassFilter = (0, _gulpFilter2.default)(['*.scss', '*.sass'], { restore: true });

  var pipeline = _gulp2.default.src(config.src).pipe(_gulpSourcemaps2.default.init()).pipe(sassFilter).pipe((0, _gulpSass2.default)({
    importer: _nodeSassJsonImporter2.default,
    includePaths: config.includePaths,
    precision: 10
  }).on('error', _gulpSass2.default.logError)).pipe(sassFilter.restore).pipe((0, _gulpAutoprefixer2.default)(config.autoprefixer)).pipe((0, _gulpPixrem2.default)())
  // Concatenate and minify styles
  .pipe((0, _gulpIf2.default)('*.css', (0, _env.prod)() ? (0, _gulpMinifyCss2.default)({
    mediaMerging: false
  }) : (0, _gulpUtil.noop)())).pipe(_gulpSourcemaps2.default.write('.')).pipe((0, _gulpFilelist2.default)(logFile, { flatten: true }));

  (0, _arrayFlatten2.default)([config.dest]).forEach(function (dest) {
    pipeline = pipeline.pipe(_gulp2.default.dest(dest));
  });

  return pipeline.pipe((0, _gulpFilter2.default)('*.css')).pipe((0, _gulpSize2.default)({ title: 'styles' }));
};
...
function load(gulp, config) {
  ...
  gulp.task('styles:clean', function (cb) {
    return (0, _deleteTracked2.default)(cb, config.dest, logFile, ['.sass-cache/']);
  });
};
