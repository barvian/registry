...
exports.watch = exports.clean = exports.build = exports.lint = exports.defaultConfig = exports.supportedExts = exports.configurable = undefined;
...
function lint() {
  ...
  return _gulp2.default.src(config.all, { since: _gulp2.default.lastRun(lint) }).pipe((0, _gulpIf2.default)('*.html', (0, _gulpHtmlExtract2.default)({ strip: true }))).pipe((0, _gulpEslint2.default)({
    parser: 'babel-eslint',
    extends: 'google'
...
function compileBundle(config, watch) {
  ...
  var rebundle = function rebundle() {
    ...
    return bundler.bundle().on('error', function (err) {
      console.error(err);
      this.emit('end');
    }).pipe((0, _vinylSourceStream2.default)(config.bundle)).pipe((0, _vinylBuffer2.default)()).pipe((0, _gulpIf2.default)(config.minify, minifyPipe())).pipe((0, _gulpMultidest2.default)(config.dest)).pipe((0, _gulpIf2.default)('*.js', watch ? (0, _browserSync.stream)() : (0, _gulpUtil.noop)()));
  };
...
function compile(config, watch) {
  var pipeline = _gulp2.default.src(config.src, { since: _gulp2.default.lastRun(compile) }).pipe((0, _gulpIf2.default)('*.html', (0, _gulpCrisper2.default)(config.crisper))).pipe((0, _gulpIf2.default)(config.sourcemaps, _gulpSourcemaps2.default.init())).pipe((0, _gulpIf2.default)(/\.(js|es6)$/, (0, _gulpBabel2.default)())).pipe((0, _gulpIf2.default)(config.minify, (0, _gulpUglify2.default)(config.uglify))).pipe((0, _gulpIf2.default)(config.sourcemaps, _gulpSourcemaps2.default.write('.'))).pipe((0, _gulpMultidest2.default)(config.dest)).pipe((0, _gulpIf2.default)('*.js', watch ? (0, _browserSync.stream)() : (0, _gulpUtil.noop)()));

  if (watch) {
    _gulp2.default.watch(config.all, function () {
      return pipeline;
    });
  }
  return pipeline;
...
function build(done, watch) {
  ...
  _gulp2.default.series(lint.bind(this), config.bundle ? compileBundle.bind(this, config, watch) : compile.bind(this, config, watch))(done);
}
