...
function process(config, watch) {
  ...
  var rebundle = function rebundle() {
    ...
    }).pipe((0, _vinylSourceStream2.default)(_path2.default.basename(config.src, _path2.default.extname(config.src)) + '.js')).pipe((0, _vinylBuffer2.default)()).pipe(_gulpSourcemaps2.default.init({ loadMaps: true })).pipe((0, _gulpUglify2.default)({ preserveComments: 'some' })).pipe(_gulpSourcemaps2.default.write('.'));
    ...
    return watch ? pipeline.pipe(_browserSync2.default.get('assets').stream()) : pipeline;
  };
