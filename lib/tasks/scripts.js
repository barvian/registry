...
function process(config) {
  var pipeline = _gulp2.default.src(config.src).pipe((0, _gulpIf2.default)(config.sourcemaps == null && !config.sourcemaps, _gulpSourcemaps2.default.init())).pipe((0, _gulpBabel2.default)()).pipe((0, _gulpIf2.default)((0, _env.prod)(), (0, _gulpUglify2.default)({ preserveComments: 'some' }))).pipe((0, _gulpIf2.default)(config.sourcemaps == null && !config.sourcemaps, _gulpSourcemaps2.default.write('.')));
...
function processBundle(config, watch) {
  ...
  var rebundle = function rebundle() {
    ...
    if ((0, _env.prod)()) {
      pipeline = pipeline.pipe(_gulpSourcemaps2.default.init()).pipe((0, _gulpUglify2.default)({ preserveComments: 'some' })).pipe(_gulpSourcemaps2.default.write());
    }
