...
function compile(config, watch) {
  ...
  return new Promise(function (res, rej) {
    ...
    return Promise.all([new Promise(function (res, rej) {
      ...
      var js = scriptTask.supportedExts.filter(function (ext) {
        return ext !== 'html';
      }).join();
      // Scripts
      scriptTask.compile({
...
function test(config, cb) {
  (0, _webComponentTester.test)({
...
function load(gulp, config) {
  ...
  gulp.task('elements:watch', function () {
    return gulp.watch([config.base + '/**/*', '!' + config.base + '/**/__tests__/**/*'], function () {
  ...
  gulp.task('elements:test', ['elements:build'], function (cb) {
    return test(config, cb);
  });
