...
function build(done) {
  var config = Object.assign({}, defaultConfig, this);
  var tmp = temp(config),
      js = scripts.supportedExts.filter(function (ext) {
    return ext !== 'html';
  }).join();
  ...
  function () {
    return _gulp2.default.src(config.base + '/**/*').pipe(_gulp2.default.dest(tmp));
  ...
  function () {
    return _gulp2.default.src(tmp + '/' + config.entry).pipe((0, _gulpVulcanize2.default)({
      inlineScripts: true,
      inlineCss: true
    })).pipe((0, _gulpCrisper2.default)()).pipe((0, _gulpIf2.default)('*.js', (0, _gulpUglify2.default)(scripts.defaultConfig.uglify))).pipe((0, _gulpMultidest2.default)(config.dest)).pipe((0, _browserSync.stream)());
  })(done);
}
